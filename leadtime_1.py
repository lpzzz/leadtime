...
# rewrite print
LOGFILE = os.path.join(os.getcwd(), 'log_1.txt')
with open(LOGFILE, 'w') as f:
    f.truncate()  # clean
buildInPrint = print
# setting for gantt
ganttfile = os.path.join(os.getcwd(), 'gantt_1.csv')
            ...
            while someting_happend:
                ...
                for i in FOUR:
                    if time_left[i] < THR:  # == 0
                        ...
                        during = processing_time[i]
                        with open(ganttfile, 'a') as f:
                            line = f'{i}, {round(timer[0] - during, 3)}, {timer[0]}, {during}, {lot}, {lots}\n'
                            f.write(line)
